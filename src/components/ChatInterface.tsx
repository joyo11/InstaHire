...
export default function ChatInterface() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update end message detection to include more phrases
      const isEndingMessage = data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("wishing you all the best") || 
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("best of luck") ||
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("have a great day") ||
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("thanks for your time") ||
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("i'll keep you updated") ||
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("next steps") ||
                            data.messages?.[data.messages.length - 1]?.content?.toLowerCase().includes("thanks for such a great conversation");
      
      if (isEndingMessage) {
        setIsInterviewFinished(true);
        setIsLatestSession(true);  // Ensure we're in latest session
      }
  ...
  const switchToConversation = async (id: string) => {
    try {
      ...
      // Check if the conversation contains an ending message
      const lastMessage = historicalMessages[historicalMessages.length - 1]?.content?.toLowerCase() || '';
      const isEndingMessage = lastMessage.includes("wishing you all the best") || 
                            lastMessage.includes("best of luck") ||
                            lastMessage.includes("have a great day") ||
                            lastMessage.includes("thanks for your time") ||
                            lastMessage.includes("i'll keep you updated") ||
                            lastMessage.includes("next steps") ||
                            lastMessage.includes("thanks for such a great conversation");
      
      // Set interview as finished if it contains an ending message
      setIsInterviewFinished(isEndingMessage);
  ...
  const handleDeleteAll = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Start new interview after deleting all
        setShowDeleteConfirm(false);
        setIsSettingsOpen(false);
        handleInitialMessage();  // Start new interview
      }
    } catch (error) {
    ...
    <div className={`flex h-screen transition-all duration-200 ${isDarkTheme ? 'bg-black text-white' : 'bg-white text-black'}`}>
                      ...
                      <input
                        ...
                        className={`font-medium text-base px-2 py-1 rounded ${
                          isDarkTheme 
                            ...
                            : 'bg-white text-[#46515b]'
                        } outline-none`}
                      />
                    ) : (
                      <div className={`font-medium text-base ${
                        isDarkTheme ? 'text-[#dcdcdc]' : 'text-[#46515b]'
                      }`}>
                        {renderConversationName(conv)}
                      </div>
                    )}
                    <div className={`text-[15px] ${
                      isDarkTheme ? 'text-[rgb(185,185,185)]' : 'text-[rgb(18,14,14)]'
                    }`}>
      ...
      <div className={`flex-1 flex flex-col transition-colors duration-10 ${isDarkTheme ? 'bg-black' : 'bg-white'}`}>
        ...
        <div className={`p-4 border-b ${isDarkTheme ? 'border-dark-light' : 'border-gray-200'} flex justify-between items-center`}>
          ...
          <div className={`text-lg font-medium ${isDarkTheme ? 'text-[#dcdcdc]' : 'text-[#46515b]'}`}>
            {conversationId ? (
          ...
          >
            <div className={`relative ${
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`absolute top-1/2 -translate-y-1/2 p-1.5 hover-effect rounded-full bg-[rgb(240,12,130)] ${
                  isInputCentered && showInitialMessage
                    ? isSidebarOpen
                      ? 'translate-x-[350px]'
                      : 'translate-x-[230px]'
                    : isSidebarOpen
                      ? 'right-[240px]'
                      : 'right-[315px]'
                } transition-smooth ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[rgb(240,12,130)]/90'}`}
              >
              ...
              {isLoading && (
                <div className={`absolute top-1/2 -translate-y-1/2 ${
                  isInputCentered && showInitialMessage
                    ...
                    : isSidebarOpen
                      ? 'right-[280px]'
                      : 'right-[360px]'  // Adjusted to stay closer to send button when sidebar is closed
                }`}>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`${
            ...
            <h3 className="text-lg font-medium mb-4">Delete All Chats?</h3>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-500 rounded hover:bg-red-600 transition-colors text-white"
              >
                Delete
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`${
            ...
            <div className="space-y-4">
              <div>
                <h4 className="font-medium mb-2">1. How do I start the interview?</h4>
              ...
              <div>
                <h4 className="font-medium mb-2">2. What happens if I don't know the answer to a question?</h4>
                <p className={isDarkTheme ? 'text-gray-300' : 'text-gray-600'}>
              ...
              <div>
                <h4 className="font-medium mb-2">3. Can I skip or end the interview early?</h4>
                <p className={isDarkTheme ? 'text-gray-300' : 'text-gray-600'}>
              ...
              <div>
                <h4 className="font-medium mb-2">4. What happens if my answer doesn't match the requirements?</h4>
                <p className={isDarkTheme ? 'text-gray-300' : 'text-gray-600'}>
              ...
              <div>
                <h4 className="font-medium mb-2">5. Can I view or revisit past interview sessions?</h4>
                <p className={isDarkTheme ? 'text-gray-300' : 'text-gray-600'}>
          ...
          <div className={`${
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-[rgb(240,12,130)] rounded hover:opacity-90 transition-colors text-white"
              >
                Switch
              </button>
            </div>
